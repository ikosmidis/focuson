...
which takes as input a fitted model and a user-supplied function of
the model parameters, and returns an estimate and a delta-method
standard error. A `confint()` method can be used to construct
inferences. Mean and median bias-corrected estimators of the focus
...
can instead be requested lazily when constructing a confidence
interval.  This evaluates the delta-method standard error at a
reconstructed parameter vector compatible with the method used to
estimate the focus parameter.
...
HulC confidence intervals can also be constructed from `focus()` objects.
The statistic is recomputed on data partitions using the same focus
specification.
...
The low-level `focus_engine()` interface works directly with a
parameter vector and other model quantities. It is intended for
advanced use and for models that are not handled by a high-level
`focus()` method.
...
Kuchibhotla A K, Balakrishnan S, Wasserman L (2023). The HulC: confidence
regions from convex hulls. *Journal of the Royal Statistical Society:
Series B*, **85**, 1461--1491. doi:10.1093/jrsssb/qkad134.
